...
copy_to_host() {
    ...
    if cat "$TMP_IMAGE" | ssh "${SSH_USER}@${host}" "podman load"; then
...
if [ "$NO_BUILD" = false ]; then
    if [ "$EXP_MXFP4" = true ]; then
        ...
        CMD=("podman" "build" "-t" "$IMAGE_TAG" "${COMMON_BUILD_FLAGS[@]}" "-f" "Dockerfile.mxfp4" ".")
...
if [ "${#COPY_HOSTS[@]}" -gt 0 ]; then
    ...
    podman save -o "$TMP_IMAGE" "$IMAGE_TAG"
