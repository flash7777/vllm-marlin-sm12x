...
turbo2 optimization: Branchless _sw_float_to_e2m1 using predicate summation
instead of if-else chain. Eliminates warp divergence on GPU.
...
__device__ __forceinline__ uint8_t _sw_float_to_e2m1(float x) {
  ...
  // Branchless E2M1: 7 comparisons summed as predicates (FSETP + IADD).
  // No branches, no warp divergence. Threshold asymmetry (<= vs <)
  // matches hardware round-to-nearest-even at midpoints.
